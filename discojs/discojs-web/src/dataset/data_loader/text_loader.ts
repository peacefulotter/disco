...
export class WebTextLoader extends dataset.loader.TextLoader {
    ...
    async load(
        ...
        worker.onmessage = (payload: globalThis.MessageEvent<any>) => {
            const sample = JSON.parse(payload.data as string) as CacheData
            cache.resolve({ value: sample.value, done: sample.done })
            worker.postMessage(JSON.stringify({ pos: 0, init: false }))
        }
