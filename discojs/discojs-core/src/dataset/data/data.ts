...
export abstract class Data {
    ...
    static async init(dataset: Dataset, task: Task, size?: number): Promise<Data> {
    ...
    get preprocessing(): (entry: tf.TensorContainer) => tf.TensorContainer {
        ...
        const preprocessingChain = applyPreprocessing.reduce(
            (acc: (x: tf.TensorContainer, task: Task) => tf.TensorContainer, fn) =>
                (x: tf.TensorContainer, task: Task) =>
                    fn(acc(x, this.task), this.task)
        )
