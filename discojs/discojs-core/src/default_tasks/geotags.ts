...
export const geotags: TaskProvider = {
    getTask(): Task {
        return {
            ...
            displayInformation: {
                ...
                summary: {
                    preview:
                        'In this challenge, we predict the geo-location of a photo given its pixels in terms of a cell number of a grid built on top of Switzerland',
                    overview:
                        'The geotags dataset is a collection of images with geo-location information used to train a machine learning algorithm to predict the location of a photo given its pixels.',
                },
                ...
                dataFormatInformation:
                    'Images should be of .png format and of size 224x224. <br> The label file should be .csv, where each row contains a file_name, class.  The class is the cell number of a the given grid of Switzerland. ',
                labelDisplay: {
    ...
    async getModel(): Promise<training.model.Model> {
        ...
        const model = new training.model.TFJSModel(
            this.getTask(),
            tf.sequential({
                layers: [
