...
export class PeerPool {
    ...
    private constructor(private readonly id: NodeID, private readonly wrtc?: WRTC) {}
    ...
    async getPeers(
        ...
        const newPeersConnections = newPeers.map(
            (peer, id) => new PeerConnection(this.id, peer, signallingServer)
        )
        ...
        await Promise.all(
            Array.from(newPeersConnections.values()).map(
                async (connection) => await connection.connect()
            )
        )
