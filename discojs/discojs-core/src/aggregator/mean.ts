...
export class MeanAggregator extends Aggregator<WeightsContainer> {
    ...
    constructor(task: Task, model?: Model, roundCutoff = 0, threshold = 1) {
    ...
    aggregate(): void {
        ...
        const result = aggregation.avg(
            this.contributions.get(0)?.values() as Iterable<WeightsContainer>
        )
        console.log(
            '[MEAN AGGREGATOR] aggregate setWeights',
            this.model === undefined
        )
        this.model?.tfjs.setWeights(result.weights)
        console.log('[MEAN AGGREGATOR] aggregate emit', result.weights.length)
        this.emit(result)
    }
