...
export class Tasks {
  ...
  constructor(private readonly config: Config, tasksAndModels: TasksAndModels) {
    ...
    this.ownRouter.post("/", (req, res) => {
      ...
      if (!(model !== undefined && newTask !== undefined && isTask(newTask))) {
        res.status(400);
        return;
      }
   ...
   */
  private async getTasksMetadata(
    request: Request,
    response: Response
  ): Promise<void> {
    response.status(200).send(this.tasksAndModels.map(([t, _]) => t).toArray());
  }
  ...
  private async getLatestModel(
    ...
    const taskAndModel = this.tasksAndModels.find(
      ([t, _]) => t.taskID === taskID
    );
